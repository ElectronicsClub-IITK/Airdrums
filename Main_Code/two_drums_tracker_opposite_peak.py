...
def detect_positive_peaks(df, column_name='acc_z', height=5, distance=None):
    peaks, _ = find_peaks(df[column_name].dropna(), height=height, distance=distance)
    return peaks

# Detect negative peaks
def detect_negative_peaks(df, column_name='acc_z', height=-5, distance=None):
    peaks, _ = find_peaks(-df[column_name].dropna(), height=-height, distance=distance)
    return peaks
...
def read_and_process_csv(csv_file, peak_detector, stop_event, interval=1):
    last_index = 0
    while not stop_event.is_set():
        ...
        if len(df) > last_index:
            ...
            # Detect peaks
            peaks = peak_detector(df_positions)
            print(f"Detected peaks in {csv_file} at:", peaks)
...
# Function to start reading and processing for two CSV files
def process_multiple_csvs(csv_files, peak_detectors):
    threads = []
    stop_event = threading.Event()
    
    try:
        for csv_file, peak_detector in zip(csv_files, peak_detectors):
            thread = threading.Thread(target=read_and_process_csv, args=(csv_file, peak_detector, stop_event))
...
csv_files = ['drum1.csv', 'drum2.csv']
peak_detectors = [detect_positive_peaks, detect_negative_peaks]
process_multiple_csvs(csv_files, peak_detectors)
